...
def sale_oper(z_b):
    ...
    print("已生成卖出订单，卖出价格为:", str(sale_price))
...
def batch_cancel(z_b):
    ...
    time.sleep(1)
    print("已撤销卖出订单，卖出价格为:", str(sale_price))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            # 主流程
            # 符合卖出条件
            if sale_min_num > float(sale_price_threshold):
                # 如果存在卖出订单
                if not no_cord_status:
                    # 如果当前订单卖出价格大于sale_min
                    if sale_price > sale_min_num :
                        batch_cancel(z_b)
                        sale_price = sale_min_num
                        sale_oper(z_b)
                else:
                    sale_price = 0.00 + sale_min_num - 0.01
                    sale_oper(z_b)
            else:
                # 如果不符合要求，如果存在卖出订单，则撤销该订单
                if not no_cord_status:
                    batch_cancel(z_b)
